...
// The owner remains authoritative through /etc/tarasec.conf.
// Example: POST action=close to /script/sshControl.php from an approved DB server.
...
function ipv4_in_cidr($ip, $cidr) {
    ...
    if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ||
        !filter_var($network, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) return false;
    $bits = (int)$bits;
    if ($bits < 0 || $bits > 32) return false;
    $ipn = ip2long($ip);
    $netn = ip2long($network);
...
function dbserver_ips($conn) {
    ...
    if ($result = $conn->query($sql)) {
        if ($row = $result->fetch_assoc()) {
            foreach (array('db1','db2','db3') as $k) {
                if (!empty($row[$k])) $ips[] = $row[$k];
            }
        }
        $result->close();
    }
...
function requester_allowed($remoteIp, $rules, $conn) {
    ...
    foreach ($items as $item) {
        if (strcasecmp($item, 'dbserver') === 0) {
            if (in_array($remoteIp, dbserver_ips($conn), true)) return true;
            continue;
        }
...
$cfg = read_tarasec_conf();
if (!cfg_on(isset($cfg['SSH_REMOTE_CONTROL']) ? $cfg['SSH_REMOTE_CONTROL'] : 'off')) {
    fail_json(403, 'remote SSH control disabled by owner');
}

$conn = getConnection();
$remoteIp = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '';
$rules = isset($cfg['SSH_REMOTE_ALLOWED_REQUESTERS']) ? $cfg['SSH_REMOTE_ALLOWED_REQUESTERS'] : '';
if ($rules === '' || !requester_allowed($remoteIp, $rules, $conn)) {
    fail_json(403, 'requester not allowed');
}

// Optional shared token. Source authorization is mandatory; the token adds a
// second check when configured. Send it in X-TaraSec-Token.
...
if ($expectedToken !== '') {
    ...
    if ($provided === '' || !hash_equals($expectedToken, $provided)) {
        fail_json(403, 'invalid remote-control token');
    }
}

$action = strtolower(trim(isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : 'status')));
if (!in_array($action, array('open','close','status'), true)) {
    fail_json(400, 'action must be open, close or status');
}
...
$response = array(
    ...
    'sshPort' => isset($cfg['SSH_PORT']) ? (int)$cfg['SSH_PORT'] : (int)$row['sshPort'],
);

if ($broadcast === 'on') {
    $response['ssh'] = $actual;
} elseif ($broadcast === 'open' || $broadcast === 'closed') {
    $response['ssh'] = $broadcast;
} elseif ($broadcast !== 'off') {
    $response['sshBroadcastError'] = 'invalid SSH_BROADCAST value';
}
