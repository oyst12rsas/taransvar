...
function latestUnitAi(array $assessment, int $ownerId, int $unitId): ?array
{
    foreach (($assessment['unit_assessments'] ?? []) as $item) {
        ...
        if ((int)($item['owner_id'] ?? 0) === $ownerId && (int)($item['unit_id'] ?? 0) === $unitId) {
            return $item;
        }
    }
    return null;
...
try {
    ...
    if (!$row) {
        $conn->close();
        statusReply(401, ['ok'=>false,'error'=>'unit_token_invalid']);
    }
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) cnt,COALESCE(MAX(severity),0) severity,MAX(COALESCE(lastSeen,created)) lastSeen FROM syslogThreat WHERE COALESCE(confirmed_unit_id,unit_id)=? AND COALESCE(lastSeen,created)>=NOW()-INTERVAL 24 HOUR");
    $stmt->bind_param('i', $unitId);
    $stmt->execute();
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        if ($candidate) {
            $ai = $candidate;
            $aiResponseId = (int)$r['aiResponseId'];
            break;
        }
    }
    $result->free();
    ...
    statusReply(200, [
        ...
        'threat'=>[
            ...
            'recentThreatRecords24h'=>$threat ? (int)$threat['cnt'] : 0,
            'maxThreatSeverity24h'=>$threatSeverity,
            'aiSeverity'=>$aiSeverity,
            'aiSummary'=>$ai ? (string)($ai['summary'] ?? '') : '',
            'aiCategory'=>$ai ? (string)($ai['category'] ?? '') : '',
