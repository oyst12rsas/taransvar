...
function senderIsRegisteredPartner($conn, $senderIp)
{
    ...
    $stmt->bind_param("s", $senderIp); $stmt->execute();
    $result = $stmt->get_result(); $registered = ($result && $result->fetch_row());
    $stmt->close(); return (bool)$registered;
}

if (!isset($_GET["f"]) || $_GET["f"] !== "request") { http_response_code(400); exit("error in parameters"); }
if (!isset($_GET["ip"], $_GET["port"])) { http_response_code(400); exit("missing params"); }

$reportedIp = hex_to_ip($_GET["ip"]);
if ($reportedIp === false || !filter_var($reportedIp, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) { http_response_code(400); exit("invalid ip"); }
