...
function managerApprovals()
{
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
        ...
        if (!hash_equals((string)$_SESSION['managerApprovalCsrf'], $csrf)) {
            ...
            if ($id > 0 && $decision === 'approve') {
                $stmt = $conn->prepare("UPDATE managerRequest SET gatewayApprovedTime=COALESCE(gatewayApprovedTime,NOW()), approvedByUserId=?, rejectedTime=NULL, active=IF(emailVerifiedTime IS NOT NULL AND credentialHash IS NOT NULL,b'1',active) WHERE managerRequestId=?");
    ...
    $sql = "SELECT managerRequestId,created,email,credentialCreatedTime,emailVerifiedTime,gatewayApprovedTime,rejectedTime,CAST(active AS UNSIGNED) active,lastUsedTime,expires FROM managerRequest ORDER BY managerRequestId DESC LIMIT 100";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (!$row['rejectedTime'] && !$row['gatewayApprovedTime']) {
            print '<form method="post" style="display:inline"><input type="hidden" name="f" value="managerApprovals"><input type="hidden" name="requestId" value="'.$id.'"><input type="hidden" name="csrf" value="'.htmlspecialchars($_SESSION['managerApprovalCsrf']).'"><button name="decision" value="approve">Approve</button> <button name="decision" value="reject">Reject</button></form>';
        } else {
            print '&nbsp;';
        }
        print '</td></tr>';
