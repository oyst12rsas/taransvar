		...
		if ($result['success']) 
		{
			...
			if (isset($data["error"]))
			{
				...
				if (isset($data["found"]) && !strcmp($data["found"], "-1"))
				{
					...
					else
					{
						...
						if (!isLanSide($szIP))
							print "<tr><td colspan=\"2\"><font color=\"red\">Server is having issues or currenly no clients visiting from LAN side. Port assignment data is currently not updated. Please try again later or try to connect on LAN side e.g by using Wireguard QR code. You are on WAN side (IP: $szIP).</font></td></tr>";
						else
			...
			else
			{
				...
				if ($data["sec"]+0 > 10000)
					...
					if ($data["sec"]+0 > 100)
						print "<tr><td><font color=\"red\">Seconds since seen:</td><td>It's ".$data["sec"]." seconds since registered data on gateway. Refreshing may help.</font></td</tr>";
