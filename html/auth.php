...
function verifyQuickLogin($phone, $mpesaCode) {
    ...
    $stmt = $conn->prepare("INSERT INTO sessions (session_id, phone, transaction_id, plan_id, expires_at, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
    $stmt->execute([$sessionId, $phone, $transaction['id'], $planId, $expiryTime]);
...
//Error if below is included.....

//    $stmt = $conn->prepare("SELECT * FROM users WHERE email = ? LIMIT 1");
 //   $stmt->execute([$email]);

    $stmt = $conn->prepare("UPDATE users SET last_login = NOW() WHERE id = ?");
//    $stmt = $conn->prepare("select id from users WHERE id = -2");
