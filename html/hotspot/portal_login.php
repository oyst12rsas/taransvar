...
if (($type === 'limited' || $type === 'expiry') && empty($user['expirytime'])) {
    ...
    if ($hours > 0) {
        $db->execute('update radcheck set expirytime=DATE_ADD(NOW(), INTERVAL '.intval($hours).' HOUR) where username=:name', array(':name' => $username));
        $user = $db->fetch('select subscriptionType,expirytime,mbquota,coalesce(mbusage,0) as mbusage from radcheck where username=:name limit 1', array(':name' => $username), PDO::FETCH_ASSOC);
        $type = (string)$user['subscriptionType'];
    }
}
...
$allowed = false;
if ($type === 'quota') $allowed = $quotaOk;
elseif ($type === 'expiry') $allowed = $expiryOk;
elseif ($type === 'limited') $allowed = $quotaOk && $expiryOk;
