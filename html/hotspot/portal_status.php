...
$row=$db->fetch('SELECT s.username,r.subscriptionType,r.expirytime,r.mbquota,COALESCE(r.mbusage,0) mbusage FROM session s LEFT JOIN radcheck r ON r.username=s.username WHERE s.ip=:ip AND s.active=1 ORDER BY s.sessionid DESC LIMIT 1', array(':ip'=>$ip), PDO::FETCH_ASSOC);
