...
function users_addtime()
{
	...
	if (!$cFetched = $pDb->fetchNext("select expirytime from radcheck where username = :name and ((op = ':=' and attribute = 'Cleartext-Password') or (op = '==' and coalesce(attribute,'') = ''))", $cFlds))
	{
	...
	if (isset($_POST["now"])) { $szSetTimeTo = $szNow; unset($_POST["now"]); }
	if (isset($_POST["hourend"])) { $szSetTimeTo = $szEndOfHour; unset($_POST["hourend"]); }
	if (isset($_POST["onehour"])) { $szSetTimeTo = $szOneHour; unset($_POST["onehour"]); }
	if (isset($_POST["24hours"])) { $szSetTimeTo = $sz24Hours; unset($_POST["24hours"]); }
	if (isset($_POST["add1hour"])) { $szSetTimeTo = $szAdd1Hour; unset($_POST["add1hour"]); }
	if (isset($_POST["add24hours"])) { $szSetTimeTo = $szAdd24Hours; unset($_POST["add24hours"]); }
