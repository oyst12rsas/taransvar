...
# 1. Owner confirmation is strong evidence because it comes from the network
# responsible for the reported source and may identify the exact stable unit.
...
# 2. High-severity normalized telemetry should shorten the normal 15-minute
# assessment interval, while the dispatcher still coalesces a burst of events.
...
# 3. Cross-owner short-window correlation is a particularly useful botnet/C2
# signal. Require stable unit identities and at least two independent owners.
