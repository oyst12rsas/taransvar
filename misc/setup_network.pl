...
# Set up DNS and Firewall Rules
	...
	#open(my $dns_conf, '>', '/etc/dnsmasq.conf') or die "Could not open file!";
	#print $dns_conf <<'END';
	my $dnsmasqConf = "interface=$szInternal
bind-interfaces
no-dhcp-interface=$szInternal
address=/#/$szInternalIP
server=8.8.8.8
server=8.8.4.4
";
	my $szConfFile = '/etc/dnsmasq.conf'; 
	open($fh, '>', $szConfFile) or die "Could not open file '$szConfFile' $!"; 
	print $fh $dnsmasqConf;
	close $fh;
