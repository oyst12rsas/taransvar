...
int elementStructSize(int nBlockDescriptor)
{
  switch (nBlockDescriptor) {
    case BLOCK_DESCRIPTIOR_SERVERS :      return sizeof(struct _ServerSpecification);
    case BLOCK_DESCRIPTIOR_INFECTIONS:    return sizeof(struct _InfectionSpecification);
    case BLOCK_DESCRIPTIOR_WHITE_LIST:    return sizeof(struct _ColoredIpSpecification);
    case BLOCK_DESCRIPTIOR_BLACK_LIST:    return sizeof(struct _ColoredIpSpecification);
    case BLOCK_DESCRIPTIOR_PARTNERS:      return sizeof(struct _PartnerSpecification);
    case BLOCK_DESCRIPTIOR_INSPECT:       return sizeof(struct _InspecitonSpecification);
    case BLOCK_DESCRIPTIOR_HONEYPORT:     return sizeof(struct _HoneyportSpecification);
    case BLOCK_DESCRIPTIOR_ASSIST:        return sizeof(struct _AssistanceRequest);
    case BLOCK_DESCRIPTIOR_DROP:          return sizeof(struct _InspecitonSpecification);
    default:  return 0;   //Should call elementStructSize(BLOCK_DESCRIPTIOR_LAST) != 0 to verify that the function is updated with new element types.
  }
}
...
void removeInfectionFromPointerList(volatile uint32_t ipAddress, volatile uint32_t ipNettmask, short port)
{
	...
	struct _Node **pNodePointer = (struct _Node **)(&pSetup->pConfigurationPointerList[BLOCK_DESCRIPTIOR_INFECTIONS]);
...
void storePartner(char *lpIP, char *lpNettmask)
{
	...
	//Check if partner is already there..
	//asdf
	for (n = 0; n < pSetup->nElementsInArray[BLOCK_DESCRIPTIOR_PARTNERS]; n++)
	{
	...
	if (n == pSetup->nElementsInArray[BLOCK_DESCRIPTIOR_PARTNERS])
	{
