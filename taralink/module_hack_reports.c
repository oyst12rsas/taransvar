		...
		*cWhat = 0;
		...
		else
		{
            ...
            *partnerError = 0;
            ...
            int globalFailed = !sendReportToGlobalDbServersVerified(&cGlobalDb, szParams, cMyIp,
                                                                     globalError, sizeof(globalError));

            if (partnerFailed || (!nRouterIp && globalFailed)) {
                char systemError[700];
                const char *detail = partnerFailed ? partnerError : globalError;
                snprintf(systemError, sizeof(systemError),
                         "Hack report delivery failed: %s:%s - %.450s",
                         row[3]?row[3]:"?", row[2]?row[2]:"?", detail);
                ...
                if (globalFailed) {
                    char systemError[700];
                    snprintf(systemError, sizeof(systemError),
                             "Hack report global copy failed after partner delivery: %s:%s - %.430s",
                             row[3]?row[3]:"?", row[2]?row[2]:"?", globalError);
                    setTaralinkSystemError(systemError, 5);
                    addWarningRecord(systemError);
                } else {
